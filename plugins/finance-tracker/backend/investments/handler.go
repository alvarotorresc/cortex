...
// Handler routes investment-related API requests to the appropriate service method.
type Handler struct {
	service *Service
}
...
func (h *Handler) Handle(req *sdk.APIRequest) (*sdk.APIResponse, error) {
	switch {
	case req.Method == "GET" && req.Path == "/investments":
		return h.list()
	case req.Method == "POST" && req.Path == "/investments":
		return h.create(req)
	case req.Method == "PUT" && strings.HasPrefix(req.Path, "/investments/"):
		return h.update(req)
	case req.Method == "DELETE" && strings.HasPrefix(req.Path, "/investments/"):
		return h.delete(req)
	default:
...
func (h *Handler) list() (*sdk.APIResponse, error) {
	investments, appErr := h.service.List()
	if appErr != nil {
		return shared.JSONError(appErr)
	}
	return shared.JSONSuccess(200, investments)
}
...
func (h *Handler) create(req *sdk.APIRequest) (*sdk.APIResponse, error) {
	...
	inv, appErr := h.service.Create(&input)
	if appErr != nil {
		return shared.JSONError(appErr)
	}
	return shared.JSONSuccess(201, inv)
}
...
func (h *Handler) update(req *sdk.APIRequest) (*sdk.APIResponse, error) {
	...
	inv, appErr := h.service.Update(id, &input)
	if appErr != nil {
		return shared.JSONError(appErr)
	}
	return shared.JSONSuccess(200, inv)
}
